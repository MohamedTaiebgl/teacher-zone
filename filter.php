						...
						<div class="inner">
							...
							<!-- Banner -->
								<section id="banner">
									<div class="content">
										<header>											
											<h2>Filtre pour les étudiants</h2>
										</header>
...
$sql = "SELECT e.*, COALESCE(SUM(p.montant), 0) as montant_paye
        FROM etudiants e , paiements p where
       
        ";

if ($localisation !== '') {
    $sql .= "  e.localisation LIKE '%$localisation%'";
}
if ($grade !== '') {
    $sql .= " AND e.grade = '$grade'";
}
if ($specialty !== '') {
    $sql .= " AND e.specialty = '$specialty'";
}
if ($status !== '') {
...
<head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
    <title>Résultats du filtre</title>
</head>
<body>
<center>
    ...
    <table border="1">
        <tr>
            ...
            <th>Numéro de téléphone des parents</th>
            <th>Localisation</th>
            <th>Classe</th>
            <th>Spécialité</th>
            <th>Statut</th>
